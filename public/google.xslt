...
<xsl:variable name="global_link_color">#a30013</xsl:variable>
<!-- <xsl:variable name="global_vlink_color">#551a8b</xsl:variable> -->
<xsl:variable name="global_vlink_color">#a30013</xsl:variable>
<!-- <xsl:variable name="global_alink_color">#ff0000</xsl:variable> -->
<xsl:variable name="global_alink_color">#a30013</xsl:variable>
...
<!-- <xsl:variable name="search_button_text">CACM Search</xsl:variable> -->
...
<xsl:variable name="res_title_color">#787e13</xsl:variable>
...
<xsl:variable name="res_url_color">#787e13</xsl:variable>
...
// </xsl:comment>
  ...
  <body onload="mTLoad('mtParametric', '{$search_url}', 1);setFocus()" dir="ltr">
...
<!-- **********************************************************************
 Search results (do not customize)
     ********************************************************************** -->
<xsl:template name="search_results_GSA">
<html>
...
<!-- Parametric Navigation code start -->
 <script language='javascript' src='http://dev.cacm.blenderbox.com/googleParametric.js'>//Comment</script>
...
<select name="site">
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='cacm_collection')]">
   <option value="cacm_collection" selected="selected">cacm_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="cacm_collection">cacm_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='csta_collection')]">
   <option value="csta_collection" selected="selected">csta_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="csta_collection">csta_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='elearn_collection')]">
   <option value="elearn_collection" selected="selected">elearn_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="elearn_collection">elearn_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='pd_collection')]">
   <option value="pd_collection" selected="selected">pd_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="pd_collection">pd_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='q_collection')]">
   <option value="q_collection" selected="selected">q_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="q_collection">q_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='queue_collection')]">
   <option value="queue_collection" selected="selected">queue_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="queue_collection">queue_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='siggraph_collection')]">
   <option value="siggraph_collection" selected="selected">siggraph_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="siggraph_collection">siggraph_collection</option>
  </xsl:otherwise>
 </xsl:choose>
</select>
...
<!-- **********************************************************************
 Spelling suggestions in result page (do not customize)
     ********************************************************************** -->
<xsl:template name="spelling_GSA">
...
<!-- **********************************************************************
 Google navigation bar in result page (do not customize)
     ********************************************************************** -->
<xsl:template name="google_navigation_GSA">
    <xsl:param name="prev"/>
    <xsl:param name="next"/>
...
<!-- BEGIN: DP custom XSL templates -->

<!-- NOTE: this overrides the (original) template "search_results", which is now named "search_results_GSA" -->
<xsl:template name="search_results">
<!-- NOTES:
 * this is content is being served within a CACM site template, so we don't need:
   * HTML, HEAD, BODY, etc.
	 * analytics
	 * page header
 * Q is always passed in, so we really don't need logic for handling missing Qs, but i've left it in anyway.
-->
...
<!-- NOTE: this overrides the (original) template "results", which is now named "results_GSA" -->
<xsl:template name="results">
<!-- NOTES:
 * removed onebox results
 * added some new variables
 * removed desktop results
 * removed bottom search box
-->
  ...
  <!-- BEGIN: new variables -->
  <xsl:variable name="start"><xsl:value-of select="RES/@EN"/></xsl:variable>
  <xsl:variable name="start_num"><xsl:value-of select="RES/@SN"/></xsl:variable>
  <xsl:variable name="num_recs"><xsl:value-of select="RES/M"/></xsl:variable>
  <!-- END: new variables -->

<script language='javascript' src='http://dev.cacm.blenderbox.com/common.js'>//Comment</script>
 <script language='javascript' src='http://dev.cacm.blenderbox.com/xmlhttp.js'>//Comment</script>
 <script language='javascript' src='http://dev.cacm.blenderbox.com/uri.js'>//Comment</script>
 <script language='javascript' src='http://dev.cacm.blenderbox.com/cluster.js'>//Comment</script>
<!-- Parametric Navigation code start -->
 <script language='javascript' src='http://dev.cacm.blenderbox.com/googleParametric.js'>//Comment</script>
...
<xsl:call-template name="my_parametric_navigation">
  ...
  <p class="SearchHeading" id="results-of">
    Results <xsl:value-of select="$start_num"/> - <xsl:value-of select="$start"/> 
    of about <xsl:value-of select="$num_recs"/> for 
    <b><xsl:value-of select="$space_normalized_query"/></b>
  </p>
  ...
  <div class="search-pagination-wrapper">
    ...
    <div class="ClearFix"><xsl:comment>do not remove</xsl:comment></div>
  </div>
  ...
  <!-- *** Filter note (if needed) *** -->
  <xsl:if test="(RES/FI) and (not(RES/NB/NU))">
    <!-- FIXME: this is kinda sketchy, as we're just removing certain params. ideally we'd just *include* certain params. -->
    <xsl:variable name="filter_params">
      <xsl:for-each select="/GSP/PARAM[
         (@name != 'filter') and
    ...
    <p id="gsa-filter-note"><i>
      In order to show you the most relevant results,
      we have omitted some entries very similar to the <xsl:value-of select="RES/@EN"/> already
      displayed.<br/>If you like, you can <a href="/search?{$filter_params}&amp;filter=0">
  ...
  <div class="search-pagination-wrapper">
    ...
    <form id="search_page_bottom" action="/search">
      <fieldset>
        <div class="field">
          <label for="site_search_query_bottom">Search Again:</label>
          <xsl:call-template name="nbsp"/>
          <input type="text" name="q" id="site_search_query_bottom" value="{$space_normalized_query}" />
          <xsl:call-template name="nbsp"/>
          <input type="image" src="/images/input.search.gif" name="submit" alt="Submit" />
        </div>
      </fieldset>
    </form>
...
<!-- NOTE: this overrides the (original) template "google_navigation", which is now named "google_navigation_GSA" -->
<xsl:template name="google_navigation">
  ...
  <xsl:variable name="num_results">10</xsl:variable>

  <!-- FIXME: this is kinda sketchy, as we're just removing certain params. ideally we'd just *include* certain params. -->
  <xsl:variable name="nav_params">
    <xsl:for-each select="/GSP/PARAM[
       (@name != 'start') and
  ...
  <xsl:if test="($prev) or ($next)">
    ...
    <xsl:choose>
      <xsl:when test="$prev">
        <a href="search?{$nav_params}&amp;start={$view_begin - $num_results - 1}">
          &#xAB;Previous <xsl:value-of select="$num_results" /> Items
        </a>
      </xsl:when>
    </xsl:choose>

    <xsl:if test="($prev) and ($next)">
      <span class="separator"><xsl:call-template name="nbsp"/></span>
    </xsl:if>

    <xsl:choose>
      <xsl:when test="$next">
        <a href="search?{$nav_params}&amp;start={$view_begin + $num_results - 1}">
          Next <xsl:value-of select="$num_results" /> Items &#xBB;
        </a>
      </xsl:when>
    </xsl:choose>
...
<!-- NOTE: this overrides the (original) template "sort_by", which is now named "sort_by_GSA" -->
<xsl:template name="sort_by">
  ...
  <xsl:variable name="sort_by_relevance_url"><xsl:value-of select="$sort_by_url"/>&amp;sort=date%3AD%3AL%3Ad1</xsl:variable>
  <xsl:variable name="sort_by_date_url"><xsl:value-of select="$sort_by_url"/>&amp;sort=date%3AD%3AS%3Ad1</xsl:variable>
...
<!-- NOTE: this overrides the (original) template "no_RES", which is now named "no_RES_GSA" -->
<xsl:template name="no_RES">
  <xsl:param name="query"/>

  <p>Your search - <b><xsl:value-of select="$query"/></b> - did not match any documents. No pages were found containing <b>"<xsl:value-of select="$query"/>"</b>.</p>
  ...
  <form id="search_page_mid" action="/search">
    <fieldset>
      <div class="field">
        <label for="site_search_query_mid">Search:</label>
        <xsl:call-template name="nbsp"/>
        <input type="text" name="q" id="site_search_query_mid" value="{$space_normalized_query}" />
        <xsl:call-template name="nbsp"/>
        <input type="image" src="/images/input.search.gif" name="submit" alt="Submit" />
      </div>
    </fieldset>
...
<!-- NOTE: this overrides the (original) template "spelling", which is now named "spelling_GSA" -->
<xsl:template name="spelling">
  <xsl:if test="/GSP/Spelling/Suggestion">
    <p>
      ...
      <a href="search?q={/GSP/Spelling/Suggestion[1]/@qe}&amp;spell=1">
...
<!-- NOTE: this overrides the (original) template match "R", which is now named "R_GSA" -->
<xsl:template match="R">
  ...
  <xsl:variable name="mt_title" select="MT[@N ='title']/@V"/>
  <xsl:variable name="mt_author" select="MT[@N ='author']/@V"/>
  <xsl:variable name="mt_date" select="MT[@N ='date']/@V"/>
  ...
  <xsl:choose>
    <xsl:when test="position() = last()">
      <xsl:text disable-output-escaping='yes'>&lt;div class=&quot;gsa-result last&quot;&gt;</xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text disable-output-escaping='yes'>&lt;div class=&quot;gsa-result&quot;&gt;</xsl:text>
    </xsl:otherwise>
  </xsl:choose>
    ...
    <p class="g">
      ...
      <xsl:if test="$show_res_title != '0'">
        <xsl:choose>
          <xsl:when test="@MIME='text/html' or @MIME='' or not(@MIME)"></xsl:when>
          <xsl:when test="@MIME='text/plain'"><b>[TEXT]</b></xsl:when>
          <xsl:when test="@MIME='application/rtf'"><b>[RTF]</b></xsl:when>
          <xsl:when test="@MIME='application/pdf'"><b>[PDF]</b></xsl:when>
          <xsl:when test="@MIME='application/postscript'"><b>[PS]</b></xsl:when>
          <xsl:when test="@MIME='application/vnd.ms-powerpoint'"><b>[MS POWERPOINT]</b></xsl:when>
          <xsl:when test="@MIME='application/vnd.ms-excel'"><b>[MS EXCEL]</b></xsl:when>
          <xsl:when test="@MIME='application/msword'"><b>[MS WORD]</b></xsl:when>
          <xsl:otherwise>
            <xsl:variable name="extension">
              <xsl:call-template name="last_substring_after">
                <xsl:with-param name="string" select="substring-after(substring-after(U,'://'),'/')"/>
                <xsl:with-param name="separator" select="'.'"/>
                <xsl:with-param name="fallback" select="'UNKNOWN'"/>
              </xsl:call-template>
            </xsl:variable>
            <b>[<xsl:value-of select="translate($extension,$lower,$upper)"/>]</b>
          </xsl:otherwise>
        </xsl:choose>
        ...
        <xsl:if test="$url_indexed">
          ...
          <xsl:choose>
            ...
            <!-- *** URI for smb or NFS must be escaped because it appears in the URI query *** -->
            <xsl:when test="$protocol='nfs' or $protocol='smb'">
              <xsl:value-of disable-output-escaping='yes' select="concat($protocol,'/', substring-after(U,'://'))"/>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of disable-output-escaping='yes' select="U"/>
            </xsl:otherwise>
        ...
        <span class="l">
          <xsl:choose>
            <!-- FIXME: this mt_title test isn't catching -->
            <xsl:when test="$mt_title != ''">
              <xsl:call-template name="reformat_keyword">
                <xsl:with-param name="orig_string" select="$mt_title"/>
              </xsl:call-template>
            </xsl:when>
            <xsl:when test="T">
              <xsl:call-template name="reformat_keyword">
    ...
    <!-- *** Meta tags *** -->
    <xsl:if test="$show_meta_tags != '0'">
      <xsl:if test="$mt_author != '' or $mt_date != ''">
        <div class="gsa-metadata">
          <xsl:value-of select="$mt_author"/>
          <xsl:if test="$mt_author != '' and $mt_date != ''"><span class="separator"> | </span></xsl:if>
          <xsl:value-of select="$mt_date"/>
        </div>
      </xsl:if>
    </xsl:if>
    ...
    <xsl:if test="HN">
      <div class="gsa-more">
        [<a class="f" href="search?{$nav_params}&amp;as_sitesearch={$crowded_url}">More results from <xsl:value-of select="$crowded_display_url"/></a>]
      </div>
    </xsl:if>
...
<!-- NOTE: this overrides the (original) template match "C", which is now named "C_GSA" -->
