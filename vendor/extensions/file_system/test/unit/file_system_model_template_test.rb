...
class FileSystemModelContentTypeTest < Test::Unit::TestCase
  
  def setup
    @template = ContentType.new :name => "A template", :content => "", :layout_id => 1
  ...
  def test_should_update_content_type_parts_and_layout_when_yaml_file
    @template.expects(:layout_name=).returns('blah')
    @template.expects(:page_class_name=).with(nil).returns(nil)
    @template.expects(:content_type_parts=).with(instance_of(Hash))
    @template.load_file(File.dirname(__FILE__) + "/../fixtures/005_some_name.yaml")
  end

  def test_content_type_parts_should_retain_order_when_loaded_from_yaml_file
    @template.expects(:layout_name=).returns('blah')
    @template.load_file(File.dirname(__FILE__) + "/../fixtures/005_some_name.yaml")
    assert_valid @template
    assert_equal "part 1", @template.content_type_parts.first.name
    assert_nil @template.content_type_parts.first.filter_id
    assert_equal "part 2", @template.content_type_parts.last.name
    assert_equal "Textile", @template.content_type_parts.last.filter_id
  end
  ...
  def test_should_save_content_and_attributes_in_separate_files
    ...
    @template.expects(:content_type_parts).returns(@parts)
  ...
  def test_content_should_be_saved_in_html_file
    ...
    @file_mock = mock()
    File.expects(:open).with("#{RAILS_ROOT}/design/content_types/a model.html", 'w').yields(@file_mock)
  ...
  def test_should_load_files
    @template_mock = mock()
    Dir.expects(:[]).with("#{RAILS_ROOT}/design/content_types/*.yaml", "#{RAILS_ROOT}/design/content_types/*.yml").returns(["#{RAILS_ROOT}/design/content_types/one.yaml"])
    ContentType.expects(:find_or_initialize_by_filename).with("#{RAILS_ROOT}/design/content_types/one.yaml").returns(@template_mock)
    @template_mock.expects(:load_file).with("#{RAILS_ROOT}/design/content_types/one.yaml")
    Dir.expects(:[]).with("#{RAILS_ROOT}/design/content_types/one*").returns(["#{RAILS_ROOT}/design/content_types/one.yaml", "#{RAILS_ROOT}/design/content_types/one.html", "#{RAILS_ROOT}/design/content_types/one_and_two.yaml", "#{RAILS_ROOT}/design/content_types/one_and_two.html"])
    @template_mock.expects(:load_file).with("#{RAILS_ROOT}/design/content_types/one.html")
    @template_mock.expects(:save!)
    ContentType.load_files
  end
end
