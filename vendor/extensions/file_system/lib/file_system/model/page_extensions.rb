module FileSystem::Model::PageExtensions
  IGNORED = %w{id lock_version draft_of parent_id layout_id
               content_type_id created_by created_at updated_by updated_at}
  ...
  module InstanceMethods
    ...
    def load_attributes(yml_file)
      ...
      content_type_name = attrs.delete('content_type_name')
      content_type = ContentType.find_by_name(content_type_name) if content_type_name
      IGNORED.each {|a| attrs.delete a }

      # NOTE: the _layout attribute doesn't exist on the Page model. not sure 
      # why it's there. removing it allows for Pages to be loaded. -JSB 2008-09-15
      #self.attributes = attrs.merge('_layout' => layout, 'content_type' => content_type).reject {|k,v| v.blank? }
      self.attributes = attrs.merge('layout' => layout, 'content_type' => content_type).reject {|k,v| v.blank? }
    end
    ...
    def save_attributes
      ...
      IGNORED.each {|a| attrs.delete a}
      attrs['content_type_name'] = self.content_type.name if self.content_type
