...
module ExceptionNotification
  ...
  def rescue_action_in_public_with_notification(exception)
    case exception
    when ActiveRecord::RecordNotFound, ActionController::UnknownController, ActionController::UnknownAction, ActionController::RoutingError, WillPaginate::InvalidPage
      status_404(request)
    else
      status_500(request)
